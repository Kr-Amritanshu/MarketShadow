import express from "express";
  ...
  function emaCalc(prices,period) {
    const k=2/(period+1),result=[prices[0]];
  ...
  function computeAllIndicators(rows) {
    ...
    const closes=rows.map(r=>r.close),highs=rows.map(r=>r.high),lows=rows.map(r=>r.low),volumes=rows.map(r=>r.volume),opens=rows.map(r=>r.open);
    ...
    const macdS=closes.map((_,i)=>emaCalc(closes.slice(0,i+1),12).at(-1)-emaCalc(closes.slice(0,i+1),26).at(-1));
    const sig=emaCalc(macdS,9).at(-1);
    results["MACD"]={value:+macd.toFixed(4),verdict:macd>sig?"Bullish":"Bearish",label:macd>sig?"Bullish crossover — momentum rising":"Bearish crossover — momentum falling",signal:macd>sig?"bullish":"bearish"};
    const bbS=closes.slice(-20),bbM=bbS.reduce((a,b)=>a+b,0)/20,bbStd=Math.sqrt(bbS.reduce((s,v)=>s+(v-bbM)**2,0)/20);
    const bbU=bbM+2*bbStd,bbL=bbM-2*bbStd,bbP=(bbU-bbL>0)?(price-bbL)/(bbU-bbL):0.5;
    ...
    const tp=rows.map(r=>(r.high+r.low+r.close)/3),vwap=tp.reduce((a,b)=>a+b,0)/tp.length;
    ...
    const tr=[],dp=[],dm=[];
    for(let i=1;i<rows.length;i++){tr.push(Math.max(highs[i]-lows[i],Math.abs(highs[i]-closes[i-1]),Math.abs(lows[i]-closes[i-1])));const u=highs[i]-highs[i-1],d=lows[i-1]-lows[i];dp.push(u>d&&u>0?u:0);dm.push(d>u&&d>0?d:0);}
    const sTR=tr.slice(-14).reduce((a,b)=>a+b,0),sDp=dp.slice(-14).reduce((a,b)=>a+b,0),sDm=dm.slice(-14).reduce((a,b)=>a+b,0);
    const diP=sTR>0?(sDp/sTR)*100:0,diM=sTR>0?(sDm/sTR)*100:0,adx=(diP+diM>0)?(Math.abs(diP-diM)/(diP+diM))*100:0;
    ...
    const rH=highs.slice(-14),rL=lows.slice(-14),hH=Math.max(...rH),lL=Math.min(...rL),stk=(hH-lL>0)?((price-lL)/(hH-lL))*100:50;
    ...
    const pH=highs.at(-2)??0,pL=lows.at(-2)??0,pC=closes.at(-2)??0,piv=(pH+pL+pC)/3,res=2*piv-pL,sup=2*piv-pH;
    ...
    const lC=closes.at(-1),lO=opens.at(-1),lH=highs.at(-1),lLow=lows.at(-1),body=Math.abs(lC-lO),tot=lH-lLow+0.001,br=body/tot,us=lH-Math.max(lC,lO),ls=Math.min(lC,lO)-lLow;
    let cp,cl,cs;
    if(br<0.1){cp="Doji";cl="Doji formed — indecision, watch closely";cs="neutral";}
    ...
    else if(br>0.7&&lC>lO){cp="Bullish Marubozu";cl="Strong bullish candle — buyers in full control";cs="bullish";}
    else if(br>0.7&&lC<lO){cp="Bearish Marubozu";cl="Strong bearish candle — sellers in full control";cs="bearish";}
  ...
  function computeConsensus(indicators) {
    ...
    const b=vals.filter(v=>v.signal==="bullish").length,bear=vals.filter(v=>v.signal==="bearish").length,n=vals.filter(v=>v.signal==="neutral").length,t=vals.length;
    const score=t>0?Math.round((b/t)*100):0;
    let verdict;
    if(b>=7)verdict="STRONGLY BULLISH";else if(b>=5)verdict="CAUTIOUSLY BULLISH";else if(bear>=7)verdict="STRONGLY BEARISH";else if(bear>=5)verdict="CAUTIOUSLY BEARISH";else verdict="NEUTRAL";
    return{verdict,score,bullish_count:b,bearish_count:bear,neutral_count:n,total:t};
  }

  function computeSmartMoneyScore(vr,pc,bo,isAno) {
    const vs=Math.min(vr/5.0,1.0)*40,sup=Math.max(0,1-Math.abs(pc)/5.0),vss=Math.min(vr/3.0,1.0)*sup*30,bs=Math.min(bo/20.0,1.0)*30,ab=isAno?5:0;
  ...
  async function analyzeStock(ticker) {
    try {
      ...
      const pt=classifyPattern(latest),bo=estimateBlockOrders(rows),sms=computeSmartMoneyScore(latest.volume_ratio,latest.price_change_pct,bo,latest.is_anomaly);
      const{label:sl,color:sc}=getScoreLabel(sms),ind=computeAllIndicators(rows),cons=computeConsensus(ind),hc=sms>=65&&cons.bullish_count>=6;
  ...
  async function runBacktest(ticker) {
    try {
      ...
      for(let i=0;i<wa.length-fw;i++){
        const row=wa[i];if(!row.is_anomaly) continue;
        const pt=classifyPattern(row);if(pt==="Normal Activity") continue;
        const ep=row.close,fp=wa.slice(i+1,i+fw+1).map(r=>r.close);
